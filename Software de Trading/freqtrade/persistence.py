...
def check_migrate(engine) -> None:
    ...
    if not has_column(cols, 'stop_loss_pct'):
        ...
        # user_id PyTradeBot
        user_id = get_column_def(cols, 'user_id', '0')
        status_buy_sell = get_column_def(cols, 'status_buy_sell', 'null')
        ...
        engine.execute(f"""insert into trades
                ...
                ticker_interval, user_id, status_buy_sell
                )
            select id, lower(exchange),
                ...
                {strategy} strategy, {ticker_interval} ticker_interval, {user_id} user_id,
                {status_buy_sell} status_buy_sell
...
class Trade(_DECL_BASE):
    ...
    status_buy_sell = Column(String, nullable=True)
    ...
    def to_json(self) -> Dict[str, Any]:
        return {
            ...
            'status_buy_sell': self.status_buy_sell,
        }
    ...
    def update(self, order: Dict) -> None:
        ...
        if order_type in ('market', 'limit') and order['side'] == 'buy':
            ...
            self.status_buy_sell = 'open'
            logger.info('%s_BUY has been fulfilled for %s.', order_type.upper(), self)
            self.open_order_id = None
        elif order_type in ('market', 'limit') and order['side'] == 'sell':
            self.close(order['price'])
            self.status_buy_sell = 'close'
    ...
    def close(self, rate: float) -> None:
        ...
        self.close_date = datetime.now(tz=timezone.utc)
        self.is_open = False
        self.open_order_id = None
        logger.info(
    ...
    def get_open_trades() -> List[Any]:
        ...
        return Trade.query \
            .filter(Trade.is_open.is_(True)) \
            .filter(Trade.user_id == int(os.getenv('id_usuario'))).all()
