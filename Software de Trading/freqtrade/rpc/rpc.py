...
class RPC:
    ...
    def _rpc_trade_statistics(
        ...
        trades = Trade.query.order_by(Trade.id) \
            .filter(Trade.user_id == int(os.getenv('id_usuario'))) \
            .all()
    ...
    def _rpc_forcesell(self, trade_id) -> Dict[str, str]:
        ...
        trade = Trade.query.filter(
            sql.and_(
                Trade.id == trade_id,
                Trade.user_id == int(os.getenv('id_usuario')),
    ...
    def _rpc_forcebuy(self, pair: str, price: Optional[float]) -> Optional[Trade]:
        ...
        trade = Trade.query.filter(Trade.is_open.is_(True)) \
    ...
    def _rpc_online(self) -> Dict[str, float]:
        """ Returns the status of trader """
        ...
        if self._freqtrade.state != State.RUNNING:
            ...
            if self._freqtrade.config['max_open_trades'] == 0:
                return {
                    'status': 'running-no-buy'
                }
            else:
                return {
                    'status': 'running'
